...
async function handleMessage(msg) {
  ...
  if (body.startsWith('#feito')) {
    ...
    if (!id || isNaN(id)) {
      await msg.reply('❌ Use: *#feito [número]* com uma foto em anexo.\nEx: *#feito 3*');
      return;
    }
    ...
    if (!pendency) {
      ...
      if (inReview) {
        ...
        await msg.reply(`⏳ A pendência *#${id}* já está com comprovante enviado e aguardando validação de *${names}*.`);
      } else {
        await msg.reply(`❌ Pendência *#${id}* não encontrada ou já foi concluída.`);
      }
      return;
    }

    if (!phoneMatch(senderPhone, pendency.responsible_phone)) {
      await msg.reply(`⛔ Apenas *${pendency.responsible_name}* pode enviar comprovante desta pendência.`);
      return;
    }
    ...
    // Muda status para AGUARDANDO VALIDAÇÃO
    db.prepare(`
      UPDATE pendencies
  ...
  if (body.startsWith('#validar')) {
    ...
    if (!id || isNaN(id)) {
      await msg.reply('❌ Use: *#validar [número]*\nEx: *#validar 3*');
      return;
    }

    if (!isValidator(db, senderPhone)) {
      const names = getValidatorNames(db) || 'Antônio ou Ronaldo';
      await msg.reply(`⛔ Apenas *${names}* podem validar pendências.`);
      return;
    }
    ...
    if (!pendency) {
      await msg.reply(`❌ Pendência *#${id}* não está aguardando validação.`);
      return;
    }
  ...
  if (body.startsWith('#rejeitar')) {
    ...
    if (!id || isNaN(id)) {
      await msg.reply('❌ Use: *#rejeitar [número] [motivo]*\nEx: *#rejeitar 3 foto ilegível*');
      return;
    }

    if (!isValidator(db, senderPhone)) {
      const names = getValidatorNames(db) || 'Antônio ou Ronaldo';
      await msg.reply(`⛔ Apenas *${names}* podem rejeitar comprovantes.`);
      return;
    }
    ...
    // Volta pra pendente, limpa comprovante
    db.prepare(`
      UPDATE pendencies
